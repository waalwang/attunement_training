...
# DEPRECATED: uses _compute_turn_weights v1 (no floor). Use _compute_example_weight_v2.
...
def _compute_example_weight_v2(
    turns: list[dict], beta: float, weight_mode: str, weight_floor: float,
) -> float:
    from data_loader import _compute_turn_weights_v2

    weights = _compute_turn_weights_v2(turns, beta, weight_mode, weight_floor)
    asst_w = [w for w in weights if w > 0]
    return float(np.mean(asst_w)) if asst_w else 1.0


# DEPRECATED: no weight floor or clamp. Use load_dpo_dataset_v2 instead.
...
# DEPRECATED: delegates to load_dpo_dataset (v1). Use load_dpo_from_config_v2.
...
def _load_shard_v2(
    ...
    for i in range(n):
        ...
        if chosen_weighting:
            row["chosen_weight"] = _compute_example_weight_v2(
...
def load_dpo_from_config_v2(config: dict) -> DatasetDict:
    dpo_cfg = config["dpo"]
    data_cfg = config["data"]
    return load_dpo_dataset_v2(
        data_dir=dpo_cfg["data_dir"],
